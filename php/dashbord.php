...
$annee = (int)date('Y'); // date renvoie string
$annee_selection = empty($_GET['annee']) ? null  : (int)$_GET['annee']; // $_GET renvoi string
...
<div class="row">
    <div class="col-md-4">
        <div class="list-group">
            <?php for($i = 0 ; $i < 5; $i++): ?>
                <!-- Clicking a link refreshes the page and adds the argument annee to the url 
                    $annee_selection gets the variables $annee - $i 
                -->
                <a class="list-group-item <?= $annee - $i === $annee_selection ? 'active' : ''; ?>" href="dashbord.php?annee=<?=$annee - $i?>">
                    <?=$annee - $i?>
                </a>
                <?php if($annee - $i === $annee_selection): ?>
                    <div class="list-group">
                    <?php foreach ($mois as $numero => $nom) : ?>
                        <a class="list-group-item <?= $numero === $mois_selection ? 'active': ''; ?>" href="dashbord.php?annee=<?= $annee_selection?>&mois=<?=$numero?>">
                            <?= $nom?>
                        </a>
                    <?php endforeach ?>
                    </div>
